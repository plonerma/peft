...
from .layer import GrowRALayer, SVDLinear


class GrowRAModel(LoraModel):
    """
    Creates GrowRA model from a pretrained transformers model. Paper:
    https://arxiv.org/abs/2308.12043

    Args:
        model ([`transformers.PreTrainedModel`]): The model to be adapted.
        config ([`GrowRAConfig`]): The configuration of the AdaLora model.
        adapter_name (`str`): The name of the adapter, defaults to `"default"`.
        low_cpu_mem_usage (`bool`, `optional`, defaults to `False`):
            Create empty adapter weights on meta device. Useful to speed up the loading process.

    Returns:
        `torch.nn.Module`: The GrowRA model.
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        # If it is not an GrowRALayer, create a new module, else update it with new adapters
        if not isinstance(target, GrowRALayer):
